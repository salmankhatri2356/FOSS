...
from db import (all_agents, agent_by_id, add_agent, set_agent_field,
                ...
                make_agent_sheet, put_setting, agent_by_tid,
...
async def aa_rate(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Sheet ban rahi hai... please wait.")
    sheet_name = make_agent_sheet(agent_name, str(agent_tid))
    if not sheet_name:
        await update.message.reply_text("Sheet nahi bani! Drive quota issue.", reply_markup=kb_admin())
        return ConversationHandler.END

    ok = add_agent({"agent_id": aid, "agent_name": agent_name, "phone": d["phone"],
                    "telegram_id": agent_tid, "sheet_name": sheet_name, "rate": rate})
    if not ok:
        await update.message.reply_text("Master sheet update nahi hui.", reply_markup=kb_admin())
        return ConversationHandler.END

    ag = {"sheet_name": sheet_name}
